...
export class ResearchAgent extends BaseAgent {
  ...
  constructor(config: Omit<AgentConfig, 'capabilities'>) {
    ...
    if (!validation.isValid) {
      throw new Error(`Missing required API keys: ${validation.missing.join(', ')}`);
    }
  ...
  protected initialize(): void {
    // Register tools
    this.registerTool('tavily_search', this.tavilyTool);
    this.registerTool('tavily_crawl', this.tavilyTool);
    
    console.log(`ResearchAgent ${this.id} initialized with tools:`, this.getTools());
    console.log(`ResearchAgent ${this.id} using model:`, this.getModelConfig().model);
    console.log(`ResearchAgent ${this.id} system prompt:`, this.getSystemPrompt());
  }
...
Your purpose is to deliver reliable, well-sourced, and clearly summarized research results.`;
  ...
  public async execute(task: any): Promise<AgentResult> {
    ...
    try {
      ...
      // Validate task
      if (!this.canHandle(researchTask)) {
        throw new Error('ResearchAgent cannot handle this task');
      }

      console.log(`ResearchAgent ${this.id} starting research on: ${researchTask.topic}`);
      ...
      if (researchTask.crawlUrl) {
        console.log(`ResearchAgent ${this.id} using crawling for URL: ${researchTask.crawlUrl}`);
        results = await this.performCrawlResearch(researchTask);
      } else {
        // Use web search for general topics
        console.log(`ResearchAgent ${this.id} using web search for topic: ${researchTask.topic}`);
        results = await this.performWebSearch(researchTask);
      }

      // Format output based on requested format
      const formattedResults = this.formatResults(results, researchTask.outputFormat || 'summary');
  ...
  private async performWebSearch(task: ResearchTask): Promise<any> {
    ...
    const summary = this.createSummaryFromResults(searchResult.result.results, task.topic);

    return {
      topic: task.topic,
      sources: searchResult.result.results,
      answer: summary,
      summary: `Research on ${task.topic} gathered from ${searchResult.result.results.length} sources.`
    };
  }

  private async performCrawlResearch(task: ResearchTask): Promise<any> {
    ...
    console.log('Crawl result structure:', JSON.stringify(Object.keys(crawlResult.result || {})));
    
    if (crawlResult.result) {
      console.log('Result pages length:', crawlResult.result.pages?.length);
      console.log('Result results length:', crawlResult.result.results?.length);
      
      if (crawlResult.result.pages && crawlResult.result.pages.length > 0) {
        console.log('First page keys:', Object.keys(crawlResult.result.pages[0]));
        console.log('First page has raw_content:', !!crawlResult.result.pages[0].raw_content);
        console.log('First page has content:', !!crawlResult.result.pages[0].content);
      }
      
      if (crawlResult.result.results && crawlResult.result.results.length > 0) {
        console.log('First result keys:', Object.keys(crawlResult.result.results[0]));
        console.log('First result has raw_content:', !!crawlResult.result.results[0].raw_content);
        console.log('First result has content:', !!crawlResult.result.results[0].content);
      }
    }
    ...
    const pages = crawlResult.result.results || crawlResult.result.pages || [];
    console.log('Using pages array with length:', pages.length);

    // Create a summary from crawled content
    const crawlSummary = this.createSummaryFromCrawlResults(pages, task.topic, task.crawlUrl);
  ...
  private createSummaryFromResults(results: any[], topic: string): string {
    ...
    topResults.forEach((result, index) => {
      if (result.content) {
        ...
        if (keyInfo.length > 50) { // Only include substantial content
  ...
  private createSummaryFromCrawlResults(pages: any[], topic: string, crawlUrl: string): string {
    ...
    const content = mainPage?.raw_content || mainPage?.rawContent || mainPage?.content || '';
    ...
    const ratingsCountMatch = content.match(/(\d+)\s+ratings/i) || content.match(/Based on \[(\d+) ratings\]/);
    if (ratingsCountMatch) {
      summary += `**Number of Ratings:** ${ratingsCountMatch[1]} student reviews\n\n`;
    }
    
    // Extract department and school
    const deptMatch = content.match(/(?:Professor in the|Computer Science|Department)/i);
    ...
    if (tagsMatch) {
      const tags = tagsMatch[1].split(/(?=[A-Z])/).filter((t: string) => t.trim().length > 2);
      if (tags.length > 0) {
  ...
  private categorizeContent(content: string, topic: string): string {
    ...
    if (lowerContent.includes('trend') || lowerContent.includes('future') || lowerContent.includes('2024') || lowerContent.includes('2025')) {
      return 'Current Trends';
    } else if (lowerContent.includes('market') || lowerContent.includes('revenue') || lowerContent.includes('growth')) {
      return 'Market Analysis';
    } else if (lowerContent.includes('technology') || lowerContent.includes('innovation') || lowerContent.includes('development')) {
      return 'Technology & Innovation';
    } else if (lowerContent.includes('application') || lowerContent.includes('use case') || lowerContent.includes('industry')) {
