...
export class FlashcardTool extends BaseTool {
  ...
  protected async executeInternal(params: any): Promise<any> {
    const { content, deckName, cardCount, difficulty } = params;
    ...
    try {
      ...
      return {
        ...
        result: {
          deck: flashcards,
          ankiData,
          downloadUrl: await this.generateDownloadUrl(ankiData, deckName || 'Generated Deck')
        }
      };
    } catch (error) {
      return {
  ...
  private async generateFlashcards(content: string, options: {
    deckName: string;
    maxCards: number;
    difficulty: string;
    ...
    const conceptCards = this.generateConceptCards(enrichedSentences, Math.ceil(minCards * 0.4));
    const definitionCards = this.generateDefinitionCards(enrichedSentences, Math.ceil(minCards * 0.3));
    const questionCards = this.generateQuestionCards(enrichedSentences, Math.ceil(minCards * 0.3));

    cards.push(...conceptCards, ...definitionCards, ...questionCards);

    // If we still don't have enough cards, generate additional ones
    if (cards.length < minCards) {
      const additionalCards = this.generateAdditionalCards(content, minCards - cards.length);
  ...
  private enrichContent(sentences: string[], content: string, minCards: number): string[] {
  ...
  private generateAdditionalCards(content: string, count: number): FlashcardData[] {
    const cards: FlashcardData[] = [];
  ...
  private generateBasicCards(content: string, count: number): FlashcardData[] {
    ...
    const importantWords = content.match(/\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b/g) || [];
  ...
  private generateConceptCards(sentences: string[], count: number): FlashcardData[] {
    const cards: FlashcardData[] = [];
  ...
  private generateDefinitionCards(sentences: string[], count: number): FlashcardData[] {
    const cards: FlashcardData[] = [];
  ...
  private generateQuestionCards(sentences: string[], count: number): FlashcardData[] {
    const cards: FlashcardData[] = [];
  ...
  private extractConcepts(sentences: string[]): Array<{term: string, definition: string}> {
  ...
  private extractDefinitions(sentences: string[]): Array<{term: string, definition: string}> {
  ...
  private generateQuestionFromSentence(sentence: string): {question: string, answer: string} | null {
  ...
  private createAnkiFormat(deck: FlashcardDeck): any {
  ...
  private async generateDownloadUrl(ankiData: any, deckName: string): Promise<string> {
   ...
   */
  public async createDownloadableFile(deck: FlashcardDeck, format: 'json' | 'csv' = 'json'): Promise<{
    filename: string;
    content: string;
    mimeType: string;
  }> {
