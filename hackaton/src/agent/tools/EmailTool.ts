...
export class EmailTool extends BaseTool {
  ...
  constructor() {
    super("email_sender", "Gmail SMTP email sending tool", {
      to: {
        type: "string",
        required: true,
      ...
      from: {
        type: "string",
        required: false,
        description: "Sender email address (defaults to configured Gmail)",
      },
      cc: {
        type: "string",
        required: false,
      ...
      isHtml: {
        type: "boolean",
        required: false,
        description: "Whether content is HTML (default: false)",
      },
    });
  ...
  protected async executeInternal(params: ToolParams): Promise<ResultData> {
    const options = params as unknown as EmailOptions;
