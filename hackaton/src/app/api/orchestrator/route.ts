...
function getOrchestrator(): AgentOrchestrator {
  if (!orchestrator) {
    ...
    const canvasAgent = new CanvasAgent({
      ...
      name: "Canvas Agent",
      description:
        "Specialized agent for Canvas LMS - access courses, assignments, grades, and announcements",
    });
    ...
    const emailAgent = new EmailAgent({
      ...
      name: "Email Agent",
      description:
        "Specialized agent for email composition and sending via Gmail SMTP",
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, task, message } = body;
    ...
    switch (action) {
      ...
      case "execute_research": {
        ...
        const researchResult = await orch.executeWithAgent(
          "research-agent-1",
          task,
        );
        return NextResponse.json({
      ...
      case "execute_canvas": {
        ...
        const canvasResult = await orch.executeWithAgent(
          "canvas-agent-1",
          task,
        );
        return NextResponse.json({
