...
export function validateApiKeys(includeOptional: boolean = false): {
  isValid: boolean;
  missing: string[];
} {
...
export function getApiKey(service: keyof typeof API_KEYS): string {
  ...
  if (!key || key.includes("your_") || key.includes("_here")) {
    throw new Error(
      `API key for ${service} is not properly configured. Please set the environment variable or update the configuration.`,
    );
  }
  return key;
}
